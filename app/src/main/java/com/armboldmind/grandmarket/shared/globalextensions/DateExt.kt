...
import com.armboldmind.grandmarket.shared.utils.DatePatterns.UTC
...
fun Date.toServerDateWithoutTimeZone(pattern: String = UTC): String {
    val sdf = SimpleDateFormat(pattern, Locale("ru"))
    return try {
        sdf.format(this)
    } catch (e: Throwable) {
...
fun String.convertServerDateTo(pattern: () -> String): String? {
    val simpleDateFormat = SimpleDateFormat(UTC, Locale("ru"))
