...
fun AppCompatActivity.withFullScreen() {
    window.apply {
        decorView.systemUiVisibility = if (Build.VERSION.SDK_INT > 27) {
            navigationBarColor = getColorCompat(R.color.dark_blue_base)
            navigationBarDividerColor = getColorCompat(R.color.dark_blue_base)
            statusBarColor = getColorCompat(android.R.color.transparent)
            View.SYSTEM_UI_FLAG_LAYOUT_STABLE or View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
        } else {
            navigationBarColor = getColorCompat(android.R.color.black)
